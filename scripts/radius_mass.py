...
# output files
embedfile = '_includes/radius_mass_embed.html'
fullfile = '_includes/radius_mass.html'
...
TOOLTIPS = [
    ...
    ("Radius", "@radius_e{0,0[.][00]} Earth; @radius_j{0,0[.][0000]} Jup"),
...
for ii, imeth in enumerate(methods):
    ...
    source = plotting.ColumnDataSource(data=dict(
            ...
            radius_j=dfcon['pl_radj'][good],
            equil_temp=dfcon['pl_eqt'][good],
            radius_sun=dfcon['st_rad'][good],
            mag_star=dfcon['st_j'][good],
            #TSM= ((((radius_e)**3)*(equil_temp))/((mass)*(radius_sun)**2))*(10**((-1(mag_star))/5)),
            #[(((('pl_rade')^3)('pl_eqt'))/(('pl_bmasse')('st_rad')^2))(10^(-1('st_tmag')/5))][good],
            url=dfcon['url'][good]
            ))
    ...
    glyph = fig.scatter('mass', 'radius_e', color=colors[ii], source=source,
                        size=8, alpha=alpha, marker=markers[ii],
                        nonselection_alpha=alpha,
                        nonselection_color=colors[ii])
    glyphs.append(glyph)
    # save the global min/max
    ymin = min(ymin, source.data['radius_e'].min())
    ymax = max(ymax, source.data['radius_e'].max())
...
fig.xaxis.axis_label = 'Mass (Earth Mass)'
